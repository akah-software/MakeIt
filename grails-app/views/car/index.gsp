		...
		<div id="list-car" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="capacity" title="${message(code: 'car.capacity.label', default: 'Capacity')}" />
					
						<g:sortableColumn property="availableSeats" title="${message(code: 'car.availableSeats.label', default: 'Available Seats')}" />
				...
				<g:each in="${carInstanceList}" status="i" var="carInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${carInstance.id}">${fieldValue(bean: carInstance, field: "name")}</g:link></td>
					
						<td>${fieldValue(bean: carInstance, field: "capacity")}</td>
					
						<td>${fieldValue(bean: carInstance, field: "availableSeats")}</td>
